...
public class HospitalController {
	...
	@GetMapping("/{hospid}")
	public ResponseEntity<?> gethospDetails(@PathVariable int hospid) {
		System.out.println("in get hospital details " + hospid);
		return ResponseEntity.ok(hospitalService.fetchhospDetails(hospid));
	}
	
	@PutMapping ("/updatehospital/{hospid}")
	public ResponseEntity<?> UpdateHospitalDetails(@RequestBody Hospital h, @PathVariable int hospid) {
		System.out.println("in update user details" + h);
		// populate the user object with existing id of the user
		h.setHospid(hospid);
		return new ResponseEntity<>(hospitalService.addorUpdateHospDetails(h), HttpStatus.CREATED);
	}
	
	@DeleteMapping("/deletehospital/{hospid}")
	public ResponseEntity<?> deleteHospitalDetais(@PathVariable int hospid) {
		System.out.println("in del hosp details " + hospid);
		return new ResponseEntity<>(hospitalService.deleteHospitalDetails(hospid), HttpStatus.OK);
	}
